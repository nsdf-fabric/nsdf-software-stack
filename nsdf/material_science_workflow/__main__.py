import os, sys,argparse,logging,time,datetime, shlex,subprocess,tempfile, shutil
...
def FixTensorFlowProblem():
	...
	logger.info(f"FixTensorFlowProblem CUDA_VISIBLE_DEVICES={CUDA_VISIBLE_DEVICES} done")
...
def Preprocess(
	...
	logger.info(f" Preprocess hdf5({rem_hdf5}) rotation-center({rotation_center}) slice-range({slice_range}) recontructions({len(s3_r)}/{tot_slices}) segmentations({len(s3_s)}/{tot_slices}) ...")
					...
					try:
						...
						# reconstruct versioning here                                               					
							...
							if white_universal is None:
								...
								import h5py
								Data_Universal = h5py.File(f"{loc}/hdf5/{white_key}", 'r')
								white_universal = Data_Universal['img_bkg']
								del Data_Universal
								sec=time.time()-t1
								logger.info(f"Loaded white in {sec} seconds")

							vmin,vmax=reconstruct_image(loc_hdf5, write_tiff_filename, I1, I2, rotation_center, white_universal, write_tiff_digit=write_tiff_digit) 

						else:
							raise Exception(f"unsupported version {reconstruction_version}")
						
						logger.info(f" Reconstruction {rem_hdf5} {I1}/{I2}vmin {vmin} vmax {vmax} loc_rec({loc_rec}) reconstruction_version({reconstruction_version})  done in {time.time()-t1} seconds")
			...
			if rem_seg in s3_s:
				...
				if not os.path.isfile(loc_seg):
					...
					if trained_model is None:
						...
						trained_model=load_new_model(GetPackageFilename("material_science_workflow/resources/seg_msd_50_2_ep100") )
						logger.info(f"Loaded trained model in {time.time()-t1} seconds")
					...
					try:
						...
						assert os.path.basename(loc_rec)==os.path.basename(loc_seg)
						bname=os.path.basename(loc_rec)
						process_whole_image(os.path.dirname(loc_rec),  os.path.dirname(loc_seg), bname, trained_model)
						assert os.path.isfile(loc_seg)
						logger.info(f" Segmentation {rem_hdf5} {I}/{tot_slices} {loc_seg} done in {time.time()-t1} seconds")
...
def CollectPreprocessStats(stats,key,tot_slices,rnum,rsize,snum,ssize):
	...
	if key in stats:
		...
		r_diff=rnum-stats[key]["rnum"];r_sec_per_slice=sec/r_diff if r_diff else 0
		s_diff=snum-stats[key]["snum"];s_sec_per_slice=sec/s_diff if s_diff else 0
...
def PreprocessMain(workflow):
	...
	if summarize:
		...
		for (rem_hdf5, tot_slices, rnum,rsize, snum,ssize) in result:
			CollectPreprocessStats(stats, os.path.basename(rem_hdf5),tot_slices,rnum,rsize,snum,ssize)
			TOT_SLICES,RNUM,SNUM,RSIZE,SSIZE=TOT_SLICES+tot_slices,RNUM+rnum,SNUM+snum,RSIZE+rsize,SSIZE+ssize
		CollectPreprocessStats(stats,"TOTAL",TOT_SLICES,RNUM,RSIZE,SNUM,SSIZE)
		logger.info("")
		sys.exit(0)
...
def ConvertImageStackMain(workflow):
	...
	keep_local_image_stack=bool(workflow["convert"]["keep-local-image-stack"])
	...
	for file in files:
		...
		whats=[]
		if not disable_reconstruction: whats.append("r")
		if not disable_segmentation:   whats.append("s")
	...
	if "dask" in workflow and bool(workflow["dask"].get("enabled",False)):
		...
		def SetupWorker():
			import os
			assert "AWS_ACCESS_KEY_ID" in os.environ # check I am getting the environment variables
			logger.info(f"DASK worker env READY")
